		...
		success : function(data, textStatus, jqXHR) {
			if ('success' == textStatus) {
				...
				$.each(data, function(i, n) {
					...
					code += '<td contentEditable="true">' + n.gdp + '</td>';
					code += '</tr> ';
				});
...
function upload() {
	...
	$(".file").change(function() {
		...
		loading("正在上传...");
		$.ajax({
			url : '/Statistic/FileUpload/gdpUpload',
			type : 'POST',
			cache : false,
...
function save() {
	...
	var Gdp = parent.eq(2 + 3 * row).text();
	var Gdpcode = parent.eq(3 * row).text();
	var url = '/Statistic/ChangeToSave/gdp';
	var json = '{';
	json += '"Gdp":"' + Gdp + '"';
	json += ',"Gdpcode":"' + Gdpcode + '"';
	json += '}';
