			...
			if ('success' == textStatus) {
				...
				$.each(data, function(i, n) {
					if (i >= start && i <= end) {
						...
						code += '<td>' + n.smcode + '</td>';
						code += '<td>' + n.smname + '</td>';
						code += '<td contentEditable="true">' + n.cntax
								+ '</td>';
						code += '</tr> ';
					} else {
						return;
	...
	bg_load = setInterval(function() {
		...
		$.each(dataObj, function(i, n) {
			...
			if (i >= start && i <= end) {
				...
				code += '<td>' + n.smcode + '</td>';
				code += '<td>' + n.smname + '</td>';
				code += '<td contentEditable="true">' + n.cntax + '</td>';
				code += '</tr> ';
			} else {
				return;
			}
...
function upload() {
	...
	$(".file").change(function() {
		...
		loading("正在上传...");
		$.ajax({
			url : '/Statistic/FileUpload/centralTaxUpload',
			type : 'POST',
			cache : false,
...
function save() {
	...
	var Cntax = parent.eq(2 + 3 * row).text();
	var Code = parent.eq(3 * row).text();
	var url = '/Statistic/ChangeToSave/centralTax';
	var json = '{';
	json += '"Cntax":"' + Cntax + '"';
	json += ',"Code":"' + Code + '"';
	json += '}';
